...
class DefaultEnvManager(EnvManager):
	...
	def remote_script(self, job):
		...
		command = "PYTHONPATH=../framework python {0} {1} --exec execute >output.txt 2>&1\n".format(appPath, params)
		...
		try:
			...
			if server.exists( outputFolder ):
				...
				if len(server.password)>0:
					client.connect( server.server_host, timeout=5.0, username=server.server_user, password=server.server_pass)
	...
	def kill(self, job):
		if job.server.exists(os.path.join(job.server.running_env, 'pid.txt') ):
			job.server.run( 'cd %s; sh ../kill.sh >> output.txt' % job.server.running_env )