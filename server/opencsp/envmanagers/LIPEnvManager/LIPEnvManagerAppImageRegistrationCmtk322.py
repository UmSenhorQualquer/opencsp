...
class LIPEnvManagerAppImageRegistrationCmtk322(LIPEnvManager):


	def __init__(self):
		super(LIPEnvManagerAppImageRegistrationCmtk322, self).__init__()
	...
	def setup_application(self, server, application):
		...
		rsync_project(
			...
			exclude=('.svn','*.pyc','output','cmtk-3.2.2'),
			delete=True,
			sshpass=True
		)
	...
	def close_setup_node(self, server, application):
		server.run('rsync -av /exper-sw/neuro/cmtk-3.2.2 /exper-sw/neuro/opencsp/applications/imageregistrationcmtk322')
