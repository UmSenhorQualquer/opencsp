...
class Client():
    ...
    def get_file_handler(self, path):
        ...
        res = self.__session.get(self.__webdav_url + path,stream = True)
        if res.status_code == 200: 
...
if __name__=='__main__':
    ...
    oc = owncloud.Client('http://localhost')
    #oc = owncloud.Client('http://storage.champalimaud.pt')
    
    oc.login('root', '123')

    #f = oc.get_file_handler('Par0000affine.txt')
    #f = oc.get_file_handler('Elastix_Trial/Par0000bspline.parm')
    for f in oc.list('/'):
        print f
